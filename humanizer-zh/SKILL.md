...
3. **高频词**：四层词表命中数，黑话层按 3.1 门控加权【cn-humanizer；虎嗅；
   实验】；
4. **句式阈值总则（误伤防护，v2）**：句式类规则均为弱信号——这些句式是人类规范修辞（对比复句/递进复句/排比/破折号国标用途 GB/T 15834—2011），人类也用，罪在过密不在使用。**单次出现不计**；单篇 ≥2 次或 ≥800 字文本密度 ≥2/千字才计（依据：108 篇实验 80–90% 篇目命中为 0、爆发集中、全场均值 0.78/千字与人类基线差距过小）。改写出口：保留最强一处、改写其余。词表类为强信号，命中即计。遗留缺陷：尚无人类写作基线组，阈值数值待校准【examples/research/06】；
5. **语体错位**：对照 assets/zh-register-map.md 的场合基线，算各类分数相对
   该场合"该有的味"的超出量。同一模型跨文体 AI 味相差可达数倍（实测约 2–8.5 倍）——错位
   程度才是主罪，不是绝对词频【实验·发现C】；
6. **思维链泄露**：按 assets/zh-thinking-leakage.md 正则清单计数【研03】。
...
- **语体错位** → 对照 assets/zh-register-map.md 对**目标场合**重校：朋友圈
  ≠公文≠公众号。错位是数倍味差的主要来源（实测约 2–8.5 倍，个别格含 0/反例）【实验·发现C】。重校方向以场合表
  "该有的味"为准，不是把一切改口语。
